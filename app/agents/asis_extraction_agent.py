# # app/agents/asis_extraction_agent.py
# from typing import List
# from langchain_core.documents import Document
# # 수정된 llm_call_service에서 gemini 호출 함수를 가져옵니다.
# from app.services.llm_call_service import call_gemini

# # 청크 관련성 판단을 위한 프롬프트 (Gemini에 최적화)
# PROMPT_FOR_CHUNK_FILTERING = """
# You are a document classifier. Your task is to determine if the following text chunk contains substantial information for creating an AS-IS system analysis report.
# If the text describes system features, architecture, user roles, data models, business processes, or technical constraints, respond with ONLY the word "relevant".
# If the text is a cover page, table of contents, legal disclaimer, or generic introduction, respond with ONLY the word "irrelevant".
...
# # 최종 보고서 생성을 위한 프롬프트 (더욱 구체화)
# PROMPT_FOR_FINAL_REPORT = """
# You are a lead system analyst creating a professional AS-IS analysis report in Korean, based on a collection of text chunks from a source document.
# The final output must be a comprehensive, well-structured Markdown document.

# Instructions:
# 1. Synthesize the provided text chunks to build a holistic understanding of the current system.
# 2. Use the following Markdown structure for the report. If information for a section is not found in the chunks, explicitly state "관련 정보를 찾을 수 없음".
# 3. Base your report strictly on the information given in the text chunks. Do not infer or invent information.
...
# ## 2. 주요 기능 및 프로세스 (Features and Processes)
# - (Describe key functionalities in detail, possibly using bullet points or tables.)
# - (If business processes are described, illustrate them logically.)

# ## 3. 시스템 구성 (System Architecture)
# - **기술 스택**: (e.g., Frontend: React, Backend: Java/Spring, DB: Oracle)
# - **아키텍처**: (e.g., Server configuration, cloud environment, MSA structure)
# - **외부 시스템 연동**: (List of integrated external systems and methods)
...
# - **성능**: (e.g., Average response time, concurrent users)
# - **보안**: (e.g., Encryption policy, access control)
# - **가용성**: (e.g., Uptime requirements, backup policy)

# ## 6. 현재 제약사항 및 문제점 (Constraints and Issues)
# - (List any known problems, technical debt, or user-reported issues.)
# ---

# Begin generating the report based on the following text chunks:

# {relevant_chunks_text}
# """

# def filter_and_generate_report(chunks: List[Document]) -> str:
#     """
#     청크(Document 객체 리스트)를 받아 필터링하고 최종 보고서를 생성합니다.
#     """
#     print("Starting chunk filtering...")
#     relevant_chunks_content = []
#     for i, chunk_doc in enumerate(chunks):
#         chunk_text = chunk_doc.page_content
#         prompt = PROMPT_FOR_CHUNK_FILTERING.format(chunk_text=chunk_text)
#         response = call_gemini(prompt)
        
#         # 응답이 null이 아니고 'relevant'를 포함하는지 확인
#         if response and "relevant" in response.lower():
#             print(f"Chunk {i+1}/{len(chunks)} is relevant.")
#             relevant_chunks_content.append(chunk_text)
#         else:
#             print(f"Chunk {i+1}/{len(chunks)} is irrelevant.")

#     if not relevant_chunks_content:
#         return "문서에서 AS-IS 분석에 유의미한 정보를 찾을 수 없습니다."

#     print(f"Found {len(relevant_chunks_content)} relevant chunks. Generating final report...")
    
#     consolidated_text = "\n\n--- // ---\n\n".join(relevant_chunks_content)
#     final_prompt = PROMPT_FOR_FINAL_REPORT.format(relevant_chunks_text=consolidated_text)
    
#     final_report = call_gemini(final_prompt)
    
#     if not final_report:
#         return "최종 보고서 생성에 실패했습니다."
